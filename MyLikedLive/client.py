...
class MainWindow(QMainWindow):
    ...
    def make_login_window(self, window):
        ...
        window.user_label = QLabel(">>> No account currently connected")
        ...
        layout.addWidget(window.connect_button, 1, 0)
        layout.addWidget(window.user_label, 1, 1, 1, 2)
    ...
    def make_results_window(self, window):
        results = "Relevant local concerts:\n" + gui_tools.list_styling(self.ss.all_concerts)
    ...
    def __init__(self):
        ...
        if os.path.exists(".cache"):
            ...
            self.login_window.user_label.setText(">>> Logged in as: " + self.ss.get_username()[:26])
            self.login_window.continue_button.setEnabled(True)
        else:
